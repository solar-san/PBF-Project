...
Maybe with summarise?

```{r}
time_index <- tourism_train %>% select(Year)

batch_NAIVE <- function(data, variable, index = time_index) {
  
  sub_tsibble <- as_tsibble(
        x = variable,
        index = time_index
      )
  
  fitted_model <- sub_tsibble %>% 
    model(
      NAIVE(
        variable
    )
